...
//! Router-bottleneck isolation test for `BlockFirst` congestion control.
//!
...
//! The slow subscriber's blocking callbacks back its router-side egress up. The
//! test asserts the *fast* subscriber is **isolated** (receives ~all the fast
//! messages produced).
//!
//! On `main` (no per-link tx-queue isolation) this is expected to **FAIL**:
//! `BlockFirst` is non-droppable, so the congested slow link blocks the
//! router's shared fan-out and starves the fast subscriber.
//!
//! Why a hard 5 s callback is enough here — unlike the sibling `Drop` test
//! (`router_congestion_drop.rs`), which needs a rate-limited TCP proxy:
//! `BlockFirst` is *non-droppable*, so a hard stall is exactly the right
//! stimulus. The slow subscriber's full egress queue makes the transport
//! *block* the shared fan-out (up to `wait_before_close`), which directly
//! fate-shares onto the fast subscriber. `Drop`, by contrast, has an
//! eager-drop fast path that *detects* a stalled link and sheds instantly,
//! releasing the shared task — so there a hard stall would *protect* the fast
//! subscriber, not starve it. Reproducing fate-sharing under `Drop` instead
//! requires a slow-but-*progressing* link that keeps the shared task busy
//! relaying within the per-fragment drop deadline; hence the proxy there.
...
// Fraction of produced fast messages the fast subscriber must receive.
...
async fn router_congestion_block_first() {
    ...
    // Three fast publisher threads, one per fast topic.
    ...
    eprintln!(
        "[router-bottleneck-block-first] produced fast={produced_fast_v} slow={produced_slow_v} | \
